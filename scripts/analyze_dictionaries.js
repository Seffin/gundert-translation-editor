...
const analyze = (options) => {
		...
		entryBlocks.forEach((block) => {
			...
			while ((sMatch = sectionRegex.exec(block)) !== null) {
				...
				sBody = sBody.replace(
					/<s>(\d{14})<\/s>/g,
					(m, r) =>
						`<span class="ref-link" data-usfm="${decodeRefWithPosition(r)}" title="${decodeRefWithPosition(r)}">${r}</span>`
				);
				sBody = sBody.replace(
				...
				sBody = sBody
					...
					.replace(/<Lemma>/g, '<em>')
					.replace(/<\/Lemma>/g, '</em> ')
					.replace(/<Transliteration>/g, '[')
					.replace(/<\/Transliteration>/g, '] ')
					.replace(/<References>/g, '<div class="ref-list">')
					.replace(/<\/References>/g, '</div>');
	...
	console.log(
		`Processed all entries with mnemonic refs preserved and hierarchy (requested lang: ${requestedLang}).`
	);
};
