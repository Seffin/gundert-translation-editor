...
console.log(
	`${colors.bright}${colors.cyan}   Gundert Translation Editor - GitHub Translation Sync Utility   ${colors.reset}`
);
console.log(
	`${colors.bright}${colors.cyan}================================================================${colors.reset}\n`
);
...
console.log(
	`${colors.blue}ℹ Target GitHub Repository: ${colors.bright}${githubOwner}/${githubRepo}${colors.reset}`
);
...
if (!targetStoryId) {
	console.log(
		`${colors.yellow}⚠ No specific story ID provided via --story. Defaulting to Malayalam Story 36 (contains active translations).${colors.reset}`
	);
	targetStoryId = '36';
}
...
async function getDrafts() {
	if (tursoUrl) {
		console.log(
			`${colors.blue}ℹ Querying cloud database for drafts: ${tursoUrl}...${colors.reset}`
		);
		try {
			...
			console.warn(
				`${colors.yellow}⚠ Failed to query Turso cloud database: ${e.message}. Falling back to JSON database.${colors.reset}`
			);
		}
	}
	...
	if (fs.existsSync(fallbackPath)) {
		console.log(
			`${colors.blue}ℹ Loading drafts from local fallback database: ${fallbackPath}...${colors.reset}`
		);
...
async function run() {
	try {
		...
		if (fs.existsSync(enFilePath)) {
			...
			console.log(
				`${colors.green}✔ Loaded English template from local path: ${enFilePath}${colors.reset}`
			);
		} else {
			const url = `https://git.door43.org/unfoldingWord/en_obs/raw/branch/master/content/${paddedStoryId}.md`;
			console.log(
				`${colors.blue}ℹ Local English file not found. Fetching from Door43: ${url}...${colors.reset}`
			);
			const res = await fetch(url);
			if (!res.ok) {
				throw new Error(
					`Failed to fetch original English story ${paddedStoryId}: ${res.statusText}`
				);
			}
		...
		if (Object.keys(translations).length === 0) {
			console.warn(
				`${colors.yellow}⚠ Warning: No translation drafts found in database for Story ${storyNumber}.${colors.reset}`
			);
		} else {
			console.log(
				`${colors.green}✔ Found ${Object.keys(translations).length} translated segments for Story ${storyNumber}.${colors.reset}`
			);
		}
		...
		const localTargetPath = path.join(
			__dirname,
			'..',
			`${targetLangPrefix}_obs`,
			'content',
			`${paddedStoryId}.md`
		);
		...
		console.log(
			`${colors.green}✔ Saved compiled markdown locally to: ${localTargetPath}${colors.reset}`
		);
		...
		try {
			...
			if (getRes.ok) {
				...
				sha = body.sha;
				console.log(
					`${colors.blue}ℹ File already exists on GitHub. Updating with SHA: ${sha}${colors.reset}`
				);
			}
		} catch (e) {
			console.log(
				`${colors.yellow}⚠ Could not check remote file existence (might be empty/new repo): ${e.message}${colors.reset}`
			);
		}
		...
		console.log(
			`${colors.bright}${colors.green}🎉 SUCCESS: Story ${paddedStoryId} synced to GitHub successfully!${colors.reset}`
		);
		console.log(
			`${colors.bright}${colors.green}================================================================${colors.reset}\n`
		);
