...
function main() {
	...
	if (!srcArg) {
		console.error(
			'Usage: node scripts/translation/run_pipeline.js --src <source.xml> --lang <code> [--provider passthrough|mock|openai-compatible] [--schema <schema.xsd>] [--draft-json <file.json>] [--policy-file <file.json>] [--policy <inline-json>]'
		);
		process.exit(2);
	}
	...
	if (!deterministicStep.ok) {
		console.log(
			JSON.stringify({ valid: false, failedStep: 'validate_deterministic', steps }, null, 2)
		);
		process.exit(1);
	}

	const llmArgs = ['--src', srcPath, '--tgt', outPath, '--lang', lang];
	if (draftJsonArg) {
