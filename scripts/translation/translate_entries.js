...
function buildReplacementQueues(draftEntry) {
	...
	(draftEntry.blocks || [])
		...
		.forEach((block) => {
			const tag =
				block.sourceTag ||
				(block.type === 'title' ? 'Title' : block.type === 'heading' ? 'Heading' : 'Paragraph');
			const text =
				block.content && typeof block.content.text === 'string' ? block.content.text : '';
			if (tag === 'Title') {
				titleQueue.push(text);
				return;
			}
			const sectionId =
				block.sourceAnchor && block.sourceAnchor.sectionId
...
function serializeDraftJsonToXml(sourceXml, draftPayload) {
	...
	const draftEntries = new Map(
		(draftPayload.entries || []).map((entry) => [String(entry.entryKey), entry])
	);
...
function buildSnippetManifestFromEntry(entryXml) {
	...
	while ((sectionMatch = sectionRegex.exec(entryXml)) !== null) {
		...
		const headingMatches = [...sectionBody.matchAll(/<Heading>([\s\S]*?)<\/Heading>/g)].map(
			(m) => m[1]
		);
		const paragraphMatches = [...sectionBody.matchAll(/<Paragraph>([\s\S]*?)<\/Paragraph>/g)].map(
			(m) => m[1]
		);
...
async function translateEntryWithFallback(
	entryXml,
	adapter,
	context,
	maxEntryChars,
	...
	translated = translated.map((text, idx) => {
		...
		if (
			srcTokens.length !== outTokens.length ||
			srcTokens.some((token, i) => token !== outTokens[i])
		) {
	...
	if (!dryRun) {
		...
		if (emitDraftJson) {
			...
			if (draftJsonScope === 'entry') {
				...
				if (!selectedEntry) {
					throw new Error(
						`Could not find entry key for draft-json-scope=entry: ${requestedEntryKey}`
					);
				}
		...
		JSON.stringify(
			{
				...
				draftJson: {
					...
					selectedEntryKey:
						draftJsonScope === 'entry'
							? args['entry-key'] || draftJsonPayload.entries[0]?.entryKey || null
							: null,
