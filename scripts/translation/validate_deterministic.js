...
function validate(srcXml, tgtXml) {
	...
	for (const tag of requiredPairs) {
		...
		if (srcCount.effectiveOpen !== srcCount.close) {
			warnings.push(
				`Source may be malformed around <${tag}> (open ${srcCount.open}, self-closing ${srcCount.selfClosing}, close ${srcCount.close}).`
			);
		}
		if (tgtCount.effectiveOpen !== tgtCount.close) {
			warnings.push(
				`Target may be malformed around <${tag}> (open ${tgtCount.open}, self-closing ${tgtCount.selfClosing}, close ${tgtCount.close}).`
			);
		}
	}
...
function main() {
	...
	if (!srcPath || !tgtPath) {
		console.error(
			'Usage: node scripts/translation/validate_deterministic.js --src <source.xml> --tgt <target.xml>'
		);
		process.exit(2);
	}
