const PROTECTED_SECTION_CONTENTS = new Set(['reference', 'references']);
...
const PROTECTED_ATTRIBUTE_NAMES = new Set(['Key', 'Content', 'target', 'Id', 'Language']);
...
function extractSectionContentSequence(xmlText) {
	return [...xmlText.matchAll(/<Section[^>]*Content="([^"]*)"/g)].map((m) =>
