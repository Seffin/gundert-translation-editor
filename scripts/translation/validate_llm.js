...
function collectEntryText(entryXml) {
	...
	while ((sectionMatch = sectionRegex.exec(entryXml)) !== null) {
		...
		const paragraphMatches = [...body.matchAll(/<Paragraph>([\s\S]*?)<\/Paragraph>/g)].map(
			(m) => m[1]
		);
...
function addIssue(report, entryReport, policy, policyKey, message, fallbackSeverity) {
	...
	const severity =
		configured === 'error' || configured === 'block' || configured === 'blocking'
			? 'error'
			: 'warning';
...
function parsePolicy(args) {
	...
	if (args.policy) {
		...
		try {
			...
			throw new Error(
				`Invalid --policy JSON value. Provide valid JSON, for example: --policy \"{\\\"paragraphCountMismatch\\\":\\\"warning\\\"}\". Received: ${String(args.policy)}`
			);
		}
...
function checkGlossaryDrift(sourceXml, targetXml, glossaryTerms) {
	...
	glossaryTerms.forEach((term) => {
		...
		if (targetCount < sourceCount) {
			mismatched += 1;
			warnings.push(
				`Glossary term drift: "${term}" appears ${sourceCount} time(s) in source but ${targetCount} time(s) in target.`
			);
		}
	});
...
async function main() {
	...
	if (!srcPath || !tgtPath) {
		console.error(
			'Usage: node scripts/translation/validate_llm.js --src <source.xml> --tgt <target.xml> [--lang fr] [--provider openai-compatible] [--draft-json <file.json>] [--policy-file <file.json>] [--policy <inline-json>]'
		);
		process.exit(2);
	}
