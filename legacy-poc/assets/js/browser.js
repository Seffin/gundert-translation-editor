...
function renderMockVersionCards(parsedRef) {
	...
	const versions = selectedIds.map((id) => BIBLE_CATALOG.find((v) => v.id === id)).filter(Boolean);
...
function renderSettingsPanel() {
	...
	document.getElementById('bsSave').addEventListener('click', () => {
		...
		if (
			viewer &&
			!viewer.classList.contains('is-hidden') &&
			refLabel &&
			refLabel.textContent !== 'Bible Viewer'
		) {
...
let contentViewMode =
	getLocalPreference('browserContentView', 'source') === 'target' ? 'target' : 'source';
...
const HUMAN_REFERENCE_PATTERN =
	/\b(?:[1-4]\s+)?(?:[A-Z]{2,}|[A-Z][a-z]+(?:\s+(?:[A-Z][a-z]+|of))*)\s+\d+:\d+(?:-\d+)?\b/g;
...
function loadTranslationState(lang) {
	...
	translationDraftsByLang[normalizedLang] = readJsonLocalStorage(
		getDraftStorageKey(normalizedLang),
		{}
	);
	translationReadyByLang[normalizedLang] = readJsonLocalStorage(
		getReadyStorageKey(normalizedLang),
		{}
	);
	return {
	...
	const renderedSections = sections
		.map((section) => {
			...
			if (isRefSection && Array.isArray(section.paragraphs)) {
				paragraphs =
					renderReferenceGroupsHtml(section.paragraphs) ||
			...
			const injected = (bySection.get(Number(section.id)) || [])
				.map(
					(block) => `
...
function getFilteredEntries() {
	...
	if (activeFilters.searchMode === 'references' && activeFilters.searchTerm) {
		...
		if (activeFilters.dictionary) {
			results = results.filter(
				(e) => (e.category || '').toLowerCase() === activeFilters.dictionary
			);
		}
	...
	if (activeFilters.searchTerm) {
		...
		results = results.filter(
			(e) => (e.title || '').toLowerCase().includes(term) || String(e.key || '').includes(term)
		);
	}
	return results;
}
...
function updateCountLabel(count) {
	...
	if (activeFilters.dictionary) {
		parts.push(
			activeFilters.dictionary.charAt(0).toUpperCase() + activeFilters.dictionary.slice(1)
		);
	}
	if (activeFilters.keyScope) {
	...
	if (countEl) {
		countEl.textContent = context ? `${count} entries in ${context}` : `${count} entries`;
	}
}
...
function initFilters() {
	...
	document.querySelectorAll('.dict-pill').forEach((pill) => {
		...
		pill.addEventListener('click', () => {
			...
			activeFilters.keyScope = '';
			document
				.querySelectorAll('.dict-pill')
				.forEach((p) => p.classList.toggle('active', p.dataset.dict === val));
	...
	document.getElementById('clearFilters').addEventListener('click', () => {
		...
		document
			.querySelectorAll('.dict-pill')
			.forEach((p) => p.classList.toggle('active', p.dataset.dict === ''));
...
function renderXmlNode(node, depth = 0) {
	...
	const attrs = Array.from(node.attributes || [])
		.map(
			(attr) =>
				` <span class="xml-attr-name">${escapeXmlText(attr.name)}</span>=<span class="xml-attr-value">&quot;${escapeXmlText(attr.value)}&quot;</span>`
		)
		.join('');
...
function renderCurrentEntryView() {
	...
	const contentHtml = isTargetView
		...
		: viewerMode === 'xml'
			? buildXmlView(entry.content)
			: enrichEntryContent(entry.content);
...
function getScopedEntries(entries, filterContext) {
	const byCategory = filterContext.category
		? entries.filter(
				(entry) => (entry.category || '').toLowerCase() === filterContext.category.toLowerCase()
			)
		: entries;
	...
	const subcategoryMatches = byCategory.filter((entry) =>
		entryMatchesSubcategory(entry, filterContext.subcategory)
	);
...
function buildScopeLabel(filterContext, scopedCount, totalCount) {
	...
	if (filterContext.keyPrefix) {
		const contextLabel = [filterContext.category, filterContext.subcategory]
			.filter(Boolean)
			.join(' > ');
...
function init() {
	if (typeof ALL_DICTIONARY_ENTRIES !== 'undefined') {
		...
		document.getElementById('entryList').innerHTML =
			'<div style="padding:20px; color:red">Error: entries.js not found or failed to load. Please ensure it is in the same folder.</div>';
	}
}
			...
			(entry, idx) => `
                ${(() => {
									const translationStatus =
										contentViewMode === 'target'
											? getEntryTranslationStatus(entry, targetLang)
											: '';
									const translationMeta = translationStatus
										? getTranslationStatusMeta(translationStatus, targetLang)
										: null;
