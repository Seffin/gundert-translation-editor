(function () {
	...
	const TARGET_ONLY_TYPES = ['target-subheading', 'target-paragraph', 'translator-note'];
	...
	const PROFILE_PROMPTS = {
		...
		default:
			'You are a Bible dictionary translation assistant. Translate prose only and preserve references, tags, and placeholders exactly.',
		literal:
			'Translate with high literal fidelity. Keep sentence structure close to source while preserving protected tokens.',
		natural:
			'Translate for natural readability while preserving all protected tokens and references exactly.'
	};

	const RUNTIME_CONFIG =
		typeof window !== 'undefined' && window.TRANSLATOR_RUNTIME_CONFIG
		...
		'openai-compatible': {
			baseUrl:
				(RUNTIME_CONFIG.providers &&
					RUNTIME_CONFIG.providers['openai-compatible'] &&
					RUNTIME_CONFIG.providers['openai-compatible'].baseUrl) ||
				'https://api.openai.com/v1',
			model:
				(RUNTIME_CONFIG.providers &&
					RUNTIME_CONFIG.providers['openai-compatible'] &&
					RUNTIME_CONFIG.providers['openai-compatible'].models &&
					RUNTIME_CONFIG.providers['openai-compatible'].models[0]) ||
				'o3',
			models: (RUNTIME_CONFIG.providers &&
				RUNTIME_CONFIG.providers['openai-compatible'] &&
				RUNTIME_CONFIG.providers['openai-compatible'].models) || ['o3', 'o4-mini', 'gpt-4.1-mini']
		},
		gemini: {
			baseUrl:
				(RUNTIME_CONFIG.providers &&
					RUNTIME_CONFIG.providers.gemini &&
					RUNTIME_CONFIG.providers.gemini.baseUrl) ||
				'https://generativelanguage.googleapis.com/v1beta/openai',
			model:
				(RUNTIME_CONFIG.providers &&
					RUNTIME_CONFIG.providers.gemini &&
					RUNTIME_CONFIG.providers.gemini.models &&
					RUNTIME_CONFIG.providers.gemini.models[0]) ||
				'gemini-2.5-pro',
			models: (RUNTIME_CONFIG.providers &&
				RUNTIME_CONFIG.providers.gemini &&
				RUNTIME_CONFIG.providers.gemini.models) || [
	...
	function syncEntryUrl() {
		...
		const category = String(state.selectedEntry.category || '')
			.trim()
			.toLowerCase();
		if (category) {
	...
	function pickThreeModels(provider, availableModels, preferredModels) {
		const available = Array.from(
			new Set((availableModels || []).map(normalizeModelName).filter(Boolean))
		);
		const preferred = Array.from(
			new Set((preferredModels || []).map(normalizeModelName).filter(Boolean))
		);
	...
	async function fetchAvailableModels(provider) {
		...
		try {
			...
			if (Array.isArray(data?.models)) {
				return data.models
					.map((m) => normalizeModelName(m.id || m.name || m.model))
					.filter(Boolean);
			}
			return null;
		} catch {
	...
	function populateModelPicker(provider, preferredModel, availableModels) {
		...
		const models =
			Array.isArray(defaults.models) && defaults.models.length > 0
				? pickThreeModels(provider, availableModels, defaults.models)
				: ['gpt-4.1-mini'];

		el.mtModel.innerHTML = models
	...
	function applyProviderDefaults(provider, forceOverride, availableModels) {
		...
		if (el.mtModel && (forceOverride || !String(el.mtModel.value || '').trim())) {
			...
			populateModelPicker(
				nextProvider,
				String(el.mtModel?.value || defaults.model),
				availableModels
			);
		}
	...
	function normalizeEntryHtmlForParsing(entryHtml) {
		return String(entryHtml || '').replace(
			...
			(_, imageKey) =>
				`<span class="tw-image-marker" data-image-key="${escapeAttr(imageKey)}"></span>`
		);
	}
	...
	function parseSections(entryHtml) {
		...
		return sectionEls.map((box, idx) => {
			const label = (box.querySelector('.section-label')?.textContent || 'other')
				.trim()
				.toLowerCase();
	...
	function renderEntryList() {
		...
		const selectedCat = state.selectedEntry
			? String(state.selectedEntry.category || '').toLowerCase()
			: null;

		el.entryList.innerHTML = state.filteredEntries
			.map((entry, idx) => {
	...
	function ensureDraftShape(draft, entry) {
		...
		return {
			...
			nextBlockId: Number.isFinite(Number(safe.nextBlockId))
				? Number(safe.nextBlockId)
				: maxExistingId + 1,
	...
	function renderSelectedEntry() {
		...
		el.targetView.innerHTML = `
            ...
            ${draft.sections
                           ...
                           ${
															s.locked
																? ''
																: `<div class="tw-source-para-actions">
			...
			sel.addEventListener('change', () => {
				...
				if (!TARGET_ONLY_TYPES.includes(nextType)) {
					renderValidationMessages([
						{ type: 'error', text: `Unsupported target-only block type: ${nextType}` }
					]);
		...
		[...el.targetView.querySelectorAll('[data-add-target-only]')].forEach((btn) => {
			btn.addEventListener('click', () => {
				...
				if (!TARGET_ONLY_TYPES.includes(type)) {
					renderValidationMessages([
						{ type: 'error', text: `Unsupported target-only block type: ${type}` }
					]);
					return;
				}
				...
				d.operations.push(
					makeOperation('insert-target-only', {
						blockId,
						sectionId,
						type
					})
				...
				d.updatedAt = new Date().toISOString();
				renderValidationMessages([
					{ type: 'ok', text: `Added ${getTargetOnlyTypeLabel(type)} block.` }
				]);
				renderSelectedEntry();
			});
		});
		...
		[...el.targetView.querySelectorAll('[data-delete-target-only]')].forEach((btn) => {
			btn.addEventListener('click', () => {
				...
				d.operations.push(
					makeOperation('delete-target-only', {
						blockId: id,
		...
		[...el.targetView.querySelectorAll('[data-delete-para]')].forEach((btn) => {
			btn.addEventListener('click', () => {
				...
				if (section.paragraphs.length <= 1) {
					renderValidationMessages([
						{ type: 'warn', text: 'Section must keep at least one paragraph.' }
					]);
					return;
				}
				section.paragraphs.splice(paragraphIndex, 1);
				d.operations.push(
					makeOperation('delete-block', {
						sectionId,
						paragraphIndex
					})
				);
	...
	function bindReferenceToggles(container) {
		container.querySelectorAll('.reference-toggle-btn').forEach((btn) => {
			btn.addEventListener('click', () => {
				...
				const refList = btn
					.closest('.reference-group')
					?.querySelector(`.reference-list[data-ref-idx="${refIdx}"]`);
				if (refList) {
	...
	function formatMnemonicReferencesForDisplay(text) {
		return String(text || '').replace(MNEMONIC_REFERENCE_PATTERN, (token) =>
			formatMnemonicReference(token)
		);
	}
	...
	function runValidation() {
		...
		if (missingRefs.length > 0) {
			messages.push({
				type: 'error',
				text: `Missing 14-digit references: ${missingRefs.slice(0, 8).join(', ')}`
			});
		}
		if (missingUsfm.length > 0) {
			messages.push({
				type: 'warn',
				text: `Missing formatted references: ${missingUsfm.slice(0, 8).join(', ')}`
			});
		}
		...
		const invalidTargetOnly = (draft.targetOnlyBlocks || []).filter(
			(b) => !TARGET_ONLY_TYPES.includes(String(b.type || ''))
		);
		if (invalidTargetOnly.length > 0) {
			messages.push({
				type: 'error',
				text: `${invalidTargetOnly.length} target-only blocks use unsupported block types.`
			});
		}
		...
		if (untouchedSections > 0) {
			messages.push({
				type: 'warn',
				text: `${untouchedSections} editable sections are unchanged from source.`
			});
		}
	...
	function renderValidationMessages(items) {
		el.validationOutput.innerHTML = items
			.map(
				(m) => `
	...
	async function translateTextWithOpenAICompatible(text, context) {
		...
		const restored = restoreMnemonicTokens(
			translated || protectedPayload.protectedText,
		...
		if (
			srcMnemonic.length !== outMnemonic.length ||
			srcMnemonic.some((token, idx) => token !== outMnemonic[idx])
		) {
			throw new Error(
				'Translation altered mnemonic references. Please retry with preserved tokens.'
			);
		}
	...
	async function machineDraftDemo() {
		...
		if (state.mtProvider === 'demo') {
			...
			renderValidationMessages([
				{
					type: 'ok',
					text: 'Machine draft demo applied. Review and validate before marking ready.'
				}
			]);
			return;
		}
		...
		if (state.mtProvider !== 'openai-compatible' && state.mtProvider !== 'gemini') {
			renderValidationMessages([
				{ type: 'error', text: `Unsupported provider: ${state.mtProvider}` }
			]);
			return;
		}
	...
	function markReady() {
		...
		renderSelectedEntry();
		renderValidationMessages([
			{ type: 'ok', text: `Entry ${entry.category} ${entry.key} marked ready.` }
		]);
	}
	...
	function filterEntries() {
		...
		if (!term) {
			...
			state.filteredEntries = state.entries.filter(
				(e) =>
					String(e.key || '')
						.toLowerCase()
						.includes(term) ||
	...
	function boot() {
		...
		if (state.filteredEntries.length > 0) {
			...
			if (preferredKey) {
				if (urlContext.category) {
					requestedIndex = state.filteredEntries.findIndex(
						(e) =>
		...
		el.resetPromptBtn?.addEventListener('click', () => {
			...
			refreshPromptBadge();
			renderValidationMessages([
				{ type: 'ok', text: `Prompt reset to built-in for "${profile}".` }
			]);
		});
