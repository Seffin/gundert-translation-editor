...
function testExtractBibleReferences() {
	...
	assert(
		refs2.some((r) => r.includes('1:1-5')),
		'Extract verse range format',
		`References: ${refs2}`
	);
	...
	assert(
		refs3.some((r) => r.includes('1 John')),
		'Extract 1 John',
		`References: ${refs3}`
	);
	assert(
		refs3.some((r) => r.includes('2 Peter')),
		'Extract 2 Peter',
		`References: ${refs3}`
	);
	...
	assert(
		refs4.some((r) => r.match(/^\d{14}$/)),
		'Extract 14-digit mnemonic',
		`References: ${refs4}`
	);
...
function testBuildBibleReferenceIndex() {
	...
	// Test index creation
	assert(
		Object.keys(bibleReferenceIndex).length > 0,
		'Index created with entries',
	...
	if (matthewRef) {
		...
		assert(
			indices.length === uniqueIndices.size,
			'No duplicate indices in reference',
...
function testSearchBibleReferences() {
	...
	assert(
		results1.length === 0,
		'Single character search returns empty',
	...
	assert(
		resultsJohn.length > 0,
		'Search for "john" finds entries',
		`Results: ${resultsJohn.length}`
	);
	assert(
		resultsJohn.some((e) => e.title === 'Lion'),
		'Lion entry found for "john"',
	...
	assert(
		results3_16.length > 0,
		'Search for "3:16" finds entries',
		`Results: ${results3_16.length}`
	);
	...
	assert(
		resultsRomans.length > 0,
		'Search for "romans" finds entries',
		`Results: ${resultsRomans.length}`
	);
	assert(
		resultsRomans.some((e) => e.title === 'Olive Tree'),
		'Olive Tree entry found for "romans"',
	...
	assert(
		resultsMatt.length === resultsMatt2.length,
	...
	if (resultsJohn.length > 0) {
		...
		assert(
			...
			'Result has title property',
			`Props: ${Object.keys(firstResult)}`
		);
		assert(
			firstResult.hasOwnProperty('content'),
			'Result has content property',
			`Props: ${Object.keys(firstResult)}`
		);
		assert(
			firstResult.hasOwnProperty('category'),
			'Result has category property',
			`Props: ${Object.keys(firstResult)}`
		);
	}
}
...
function testIntegration() {
	...
	assert(
		filtered.some((e) => e.title === 'Lion'),
		'Lion entry found in reference search',
	...
	assert(
		...
		'Can filter reference results by dictionary',
		`Found ${filteredFlora.length} entries`
	);
	assert(
		filteredFlora.every((e) => e.category === 'flora'),
		'All results are flora',
	...
	assert(
		entriesSearch.length > 0,
		'Entry search mode still works',
		`Found ${entriesSearch.length} entries`
	);
	assert(
		entriesSearch.some((e) => e.title === 'Lion'),
		'Lion entry found in entry search',
...
function testEdgeCases() {
	...
	assert(
		Object.keys(bibleReferenceIndex).length === 0,
		'Handles entries with no references',
	...
	assert(
		...
		`Found ${longResults.length} entries`
	);
}
...
function testImagePathResolution() {
	...
	assert(
		candidates[0] === '../data/images/WEB-0066_bears.jpg',
		'First path prefers new data/images location',
		`First: ${candidates[0]}`
	);
	assert(
		candidates.some((p) => p === 'images/WEB-0066_bears.jpg'),
		'Legacy images path retained as fallback',
		'Found legacy fallback'
	);

	const explicit = resolveImageCandidates('WEB-0066_bears.png');
	assert(
		explicit[0] === '../data/images/WEB-0066_bears.png',
		'Explicit extension uses new data/images path',
		`First: ${explicit[0]}`
	);
	assert(
		explicit.includes('images/WEB-0066_bears.png'),
		'Explicit extension keeps legacy fallback path',
		'Found explicit legacy fallback'
	);
}
...
function testCardRenderingStructure() {
	...
	assert(
		Boolean(firstHierarchy),
		'Card includes hierarchy block',
		`Exists: ${Boolean(firstHierarchy)}`
	);

	const categoryTag = cards[0].querySelector('.meta-tag');
	assert(
		Boolean(categoryTag) && categoryTag.textContent.includes('Animals, general'),
		'Category tag renders parent section title',
		`Text: ${categoryTag ? categoryTag.textContent : ''}`
	);

	const subTag = cards[0].querySelector('.meta-tag.sub');
	assert(
		Boolean(subTag) && subTag.textContent.includes('Domestic animals'),
		'Subcategory tag renders entry section title',
		`Text: ${subTag ? subTag.textContent : ''}`
	);

	const secondCategoryTag = cards[1].querySelector('.meta-tag');
	assert(
		Boolean(secondCategoryTag) && secondCategoryTag.textContent.includes('Wild Trees and Shrubs'),
		'Category lookup is dictionary-scoped (no cross-category key collision)',
		`Text: ${secondCategoryTag ? secondCategoryTag.textContent : ''}`
	);
...
function testXmlViewerRendering() {
	...
	assert(
		typeof rendered === 'string' && rendered.includes('class="xml-viewer"'),
		'XML view wrapper is rendered',
		'Found xml-viewer wrapper'
	);
	assert(
		rendered.includes('class="xml-node"'),
		'XML node details blocks are rendered',
		'Found xml-node details'
	);
	assert(
		rendered.includes('class="xml-summary"'),
		'XML summary rows are rendered',
		'Found xml-summary'
	);
	assert(
		rendered.includes('class="xml-tag">div</span>'),
		'Tag names are syntax-highlighted',
		'Found xml-tag span for div'
	);
	assert(
		rendered.includes('class="xml-attr-name">data-kind</span>'),
		'Attribute names are syntax-highlighted',
		'Found data-kind attribute'
	);
	assert(
		rendered.includes('&amp;quot;references&amp;quot;') ||
			rendered.includes('&quot;references&quot;'),
		'Attribute values are escaped in output',
		'Found escaped attribute value'
	);

	const escaped = escapeXmlText('<tag x="1">Tom & Jerry</tag>');
	assert(
		escaped.includes('&lt;tag'),
		'escapeXmlText escapes opening brackets',
		`Escaped: ${escaped}`
	);
	...
	assert(
		emptyRendered.includes('No XML content found for this entry.'),
		'Empty XML view shows friendly message',
	...
	assert(
		children.length >= 2,
		'Renderable XML children include elements and non-empty text only',
