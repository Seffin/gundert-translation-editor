...
export async function POST(req: NextRequest) {
	try {
		...
		if (provider === 'gemini') {
			...
			const model = String(
				rawBody.model || payload.model || providerConfig.model || 'gemini-2.5-pro'
			).trim();
				...
				contents: [
					{
						parts: [
							{
								text:
									payload.messages?.[payload.messages.length - 1]?.content || payload.prompt || ''
							}
						]
					}
				],
...
function resolveProviderConfig(provider: string, rawBody: Record<string, any>) {
...
async function normalizeBody(req: NextRequest): Promise<Record<string, any>> {
	try {
