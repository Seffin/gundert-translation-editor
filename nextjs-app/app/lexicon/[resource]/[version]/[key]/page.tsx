...
import { fetchEntryBySlugVersionKey, type TranslationStatus } from '@/lib/browser/entry-list';
		...
		<div className="min-h-screen bg-slate-50">
			...
			<header className="border-b border-slate-200 bg-white px-4 py-3 shadow-sm">
				<div className="mx-auto flex max-w-4xl items-center gap-3">
					...
					<div className="ml-auto">
						<span
							className={`rounded-full px-2.5 py-1 text-xs font-medium ring-1 ${STATUS_CHIP[entry.translationStatus]}`}
						>
			...
			<main className="mx-auto max-w-4xl px-4 py-8 space-y-6">
				...
				<div>
					...
					{sc?.intro && <p className="mt-2 text-sm text-slate-600 italic">{sc.intro}</p>}
				</div>
				...
				<footer className="rounded-xl border border-slate-200 bg-white px-5 py-4 text-xs text-slate-500 space-y-1">
					<div className="flex gap-6">
						<span>
							Source language: <strong className="text-slate-700">{entry.sourceLanguage}</strong>
						</span>
						<span>
							Updated:{' '}
						...
						{sc?.metadata?.checksum && (
							<span>
								...
								<code className="font-mono text-slate-400">
									{sc.metadata.checksum.slice(0, 12)}…
								</code>
							</span>
						)}
					</div>
