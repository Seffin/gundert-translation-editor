...
describe('Import Workflow Risk Matrix (Red Phase)', () => {
	it('IMP-001: should surface structured error code for missing source files', async () => {
		await expect(parseUBSXml('Z:/non-existent/missing-fauna.xml')).rejects.toHaveProperty(
			'code',
	...
	it('IMP-003: should reject malformed XML with IMPORT_XML_NOT_WELL_FORMED', async () => {
		const filePath = await writeTempXml(
			'<Thematic_Lexicon><ThemLex_Entry></Thematic_Lexicon>',
			'malformed.xml'
		);
		await expect(parseUBSXml(filePath)).rejects.toHaveProperty(
			'code',
	...
	it('IMP-004: should fail schema-invalid XML with IMPORT_XML_SCHEMA_INVALID', async () => {
		const filePath = await writeTempXml(
			'<?xml version="1.0"?><Thematic_Lexicon><ThemLex_Entry/></Thematic_Lexicon>',
			'schema-invalid.xml'
		);
		const parserModule = await import('@/lib/xml-parser/parser');
		expect(typeof (parserModule as any).validateAgainstXsd).toBe('function');
		await expect((parserModule as any).validateAgainstXsd(filePath)).rejects.toHaveProperty(
			'code',
...
<Thematic_Lexicon>
  <ThemLex_Entry Key="1"><Title>A</Title><Index><IndexItem><l target="FAUNA:999">Missing</l></IndexItem></Index></ThemLex_Entry>
		...
		await expect((parserModule as any).validateIndexTargets(filePath)).rejects.toHaveProperty(
			'code',
...
<Thematic_Lexicon>
  ...
  <ThemLex_Entry Key="1"><Title>B</Title></ThemLex_Entry>
		...
		await expect((parserModule as any).validateDuplicateKeys(filePath)).rejects.toHaveProperty(
			'code',
...
<Thematic_Lexicon xmlns="urn:wrong:namespace">
  <ThemLex_Entry Key="1"><Title>A</Title></ThemLex_Entry>
		...
		await expect((parserModule as any).validateNamespace(filePath)).rejects.toHaveProperty(
			'code',
	...
	it('IMP-012: should enforce parse timeout on oversized files', async () => {
		...
		await expect(
			(parserModule as any).parseUBSXmlWithLimits(filePath, { timeoutMs: 100 })
		).rejects.toHaveProperty('code', 'IMPORT_TIMEOUT');
	});
