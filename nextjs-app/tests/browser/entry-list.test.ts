...
describe('parseResourcesFromQuery', () => {
	...
	it('trims surrounding whitespace for each slug', () => {
		expect(parseResourcesFromQuery(' ubs-fauna ,  ubs-flora ')).toEqual(['ubs-fauna', 'ubs-flora']);
	});
	...
	it('preserves duplicate slugs', () => {
		expect(parseResourcesFromQuery('ubs-fauna,ubs-fauna')).toEqual(['ubs-fauna', 'ubs-fauna']);
	});
...
describe('getResourcePriority', () => {
	...
	it('returns stable order: UBS < unfoldingWord < SIL < Tyndale', () => {
		const [ubs, uw, sil, tyn] = ['UBS', 'unfoldingWord', 'SIL', 'Tyndale'].map(getResourcePriority);
...
const shouldRunDbIntegration =
	process.env.RUN_DB_INTEGRATION_TESTS === 'true' && Boolean(process.env.POSTGRES_URL_NON_POOLING);
...
describeDbIntegration('GH-009: fetchEntryListWithStatus - Resource Filter Integration', () => {
	...
	afterAll(async () => {
		...
		await db
			.delete(resourceEntries)
			.where(eq(resourceEntries.resource_version_id, testFaunaVersionId));
		await db
			.delete(resourceEntries)
			.where(eq(resourceEntries.resource_version_id, testFloraVersionId));
		await db
			.delete(resourceEntries)
			.where(eq(resourceEntries.resource_version_id, testRealiaVersionId));
	...
	it('combines resource filter with status filter correctly', async () => {
		const result = await fetchEntryListWithStatus(null, 'ml', ['draft', 'approved'], 50, [
			'ubs-fauna'
		]);
		const testEntries = filterTestEntries(result.entries);
		expect(testEntries.every((e) => e.resourceSlug === 'ubs-fauna')).toBe(true);
