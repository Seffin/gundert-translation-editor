...
export async function parseUBSXml(filePath: string): Promise<ParsedEntry[]> {
	...
	if (!parsed || !parsed.Thematic_Lexicon) {
		throw new ImportWorkflowError(
			'IMPORT_XML_NOT_WELL_FORMED',
			'Missing Thematic_Lexicon root element',
			'parse'
		);
	}
	...
	const themLexEntries = Array.isArray(rawEntries) ? rawEntries : rawEntries ? [rawEntries] : [];
 ...
 */
function parseSections(sectionsElement: any): Array<{
	type: string | null;
	content: string | null;
	heading: string | null;
	subheading: string | null;
 ...
 */
function parseIndexItems(indexElement: any): Array<{ target: string; label: string }> {
...
async function readAndValidateXml(filePath: string, limits: XmlLimits): Promise<string> {
	...
	if (data.length > limits.maxBytes) {
		throw new ImportWorkflowError(
			'SEC_SIZE_LIMIT_EXCEEDED',
			`XML exceeds ${limits.maxBytes} bytes`,
			'read'
		);
	}
	...
	try {
		...
		throw new ImportWorkflowError(
			'IMPORT_ENCODING_INVALID',
			'Invalid UTF-8 encoding in XML input',
			'decode'
		);
	}
	...
	if (!xmlContent.trimStart().startsWith('<')) {
		throw new ImportWorkflowError(
			'SEC_CONTENT_TYPE_INVALID',
			'Input does not appear to be XML content',
			'validate'
		);
	}
	...
	if (depth > limits.maxDepth) {
		throw new ImportWorkflowError(
			'SEC_MAX_DEPTH_EXCEEDED',
			`XML nesting depth exceeds ${limits.maxDepth}`,
			'validate'
		);
	}
...
export async function parseUBSXmlWithLimits(
	...
	const timeoutPromise = new Promise<ParsedEntry[]>((_, reject) => {
		const timer = setTimeout(() => {
			reject(
				new ImportWorkflowError('IMPORT_TIMEOUT', `Import timed out after ${timeoutMs}ms`, 'limits')
			);
		}, timeoutMs);
...
export async function validateAgainstXsd(filePath: string): Promise<void> {
	...
	if (entries.length === 0) {
		throw new ImportWorkflowError(
			'IMPORT_XML_SCHEMA_INVALID',
			'No ThemLex_Entry elements found',
			'schema'
		);
	}
	...
	for (const entry of entries) {
		if (!entry.key || !entry.title) {
			throw new ImportWorkflowError(
				'IMPORT_XML_SCHEMA_INVALID',
				'Entry is missing required Key or Title',
				'schema',
				entry.key
			);
		}
	...
	for (const entry of entries) {
		for (const idx of entry.index) {
			...
			if (targetKey && !keys.has(targetKey)) {
				throw new ImportWorkflowError(
					'IMPORT_REFERENCE_INVALID',
					`Index target ${idx.target} is missing`,
					'validate',
					entry.key
				);
			}
...
export function canonicalXmlDiff(
	sourceXml: string,
	targetXml: string
...
export async function validateNamespace(filePath: string): Promise<void> {
	...
	if (hasWrongNamespace && !xml.includes(EXPECTED_NAMESPACE)) {
		throw new ImportWorkflowError(
			'IMPORT_NAMESPACE_INVALID',
			'Unexpected namespace for Thematic_Lexicon root',
			'schema'
		);
	}
}
...
export function resolveImportPathSafely(
	candidatePath: string,
	baseDir = path.join(process.cwd(), 'data', 'xml')
): string {
...
export function toSafeClientError(error: unknown): { code: string; message: string } {
	...
	return {
		code: 'IMPORT_XML_NOT_WELL_FORMED',
		message: 'Import failed. Please check file format and try again.'
	};
}
